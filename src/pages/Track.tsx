      ...
      <div className="min-h-screen bg-white">
        ...
        {!order && (
          <div className="text-center mb-10">
            ...
            <h1 className="text-4xl font-black text-black mb-3 tracking-tight">Track Your Order</h1>
            <p className="text-gray-600 text-base max-w-2xl mx-auto">Enter your order ID to get real-time updates and delivery status</p>
          </div>
        )}

        {/* Search Box - Compact when order is displayed */}
        <div className={`mx-auto transition-all duration-500 ${order ? 'max-w-full mb-6' : 'max-w-2xl mb-10'}`}>
          <div className={`bg-white rounded-xl border overflow-hidden transition-all duration-500 ${order ? 'border-gray-300 shadow' : 'border-gray-200 shadow-lg'}`}>
            <div className={`transition-all duration-500 ${order ? 'p-4' : 'p-6'}`}>
          ...
          <input
            ...
            onChange={(e) => setInput(e.target.value)}
                    ...
                    className={`w-full px-5 border-2 border-gray-300 rounded-lg outline-none focus:border-black transition-all font-semibold ${order ? 'py-2.5 text-sm' : 'py-3 text-base'}`}
                  />
                  {!order && (
                    <div className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-400 text-xs font-mono">
                      RS-XXXX-XXXX
                    </div>
                  )}
                </div>
          <button
            onClick={() => handleSearch()}
                  className={`bg-black hover:bg-gray-900 text-white rounded-lg font-bold transition-all whitespace-nowrap flex items-center justify-center gap-2 ${order ? 'px-6 py-2.5 text-sm' : 'px-8 py-3 text-base'}`}
          >
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="2">
                    <path strokeLinecap="round" strokeLinejoin="round" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
                  </svg>
                  {order ? 'New Search' : 'Track'}
          </button>
              </div>
              {!order && (
          ...
          <div className="grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
            ...
            <div className="space-y-6">
              ...
              <div className="bg-white rounded-2xl shadow-xl border border-gray-200 overflow-hidden">
                <div className="relative bg-gradient-to-br from-black via-gray-900 to-black p-6 text-white overflow-hidden">
                    ...
                    <div className="flex items-start justify-between gap-4 mb-4">
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-3">
                          ...
                          <div className="text-xs text-gray-400 uppercase tracking-wide font-bold">Order Details</div>
                        </div>
                        <div className="text-2xl font-black mb-1 tracking-tight">#{order.id}</div>
                  ...
                  >
                    ...
                    <div className="grid grid-cols-3 gap-4 pt-4 border-t border-white/10">
                      <div className="text-center">
                        <div className="text-xl font-black mb-0.5">{order.items.length}</div>
                        <div className="text-[10px] text-gray-400 uppercase tracking-wide">Items</div>
                      </div>
                      <div className="text-center border-x border-white/10">
                        <div className="text-xl font-black mb-0.5">Rs. {(subtotal + shipping).toLocaleString("en-PK")}</div>
                        <div className="text-[10px] text-gray-400 uppercase tracking-wide">Total</div>
                      </div>
                      <div className="text-center">
                        <div className="text-xl font-black mb-0.5">{shipping === 0 ? "FREE" : "PAID"}</div>
                        <div className="text-[10px] text-gray-400 uppercase tracking-wide">Shipping</div>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div className="bg-white rounded-2xl shadow-xl border border-gray-200 p-6">
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <div>
                    <h3 className="font-black text-lg text-black">Delivery Timeline</h3>
                    <p className="text-xs text-gray-600">Track every step</p>
                  </div>
                </div>
                <div className="relative">
                  {/* Timeline line */}
                  <div className="absolute left-[11px] top-2 bottom-2 w-0.5 bg-gradient-to-b from-black via-gray-400 to-gray-200 rounded-full" />
                  ...
                  <ul className="space-y-4 relative">
                    {STATUS_STEPS.map((s) => {
                      ...
                      return (
                        ...
                        >
                          <span
                            className={`relative z-10 inline-flex items-center justify-center w-6 h-6 rounded-lg border-2 shadow-md transition-all ${
                              done 
                                ? isActive
                          ...
                          <div className="flex-1 pb-1">
                            ...
                            <div className={`text-xs ${done ? "text-gray-600" : "text-gray-400"}`}>
                              {done ? fmtDate(tl[s]) : "Pending"}
                            </div>
                          </div>
                        </li>
                      );
            ...
            <aside className="space-y-6">
              ...
              <div className="bg-white rounded-2xl shadow-xl border border-gray-200 p-5">
                <div className="flex items-center gap-3 mb-4">
                  <div className="w-9 h-9 bg-gradient-to-br from-black to-gray-800 rounded-xl flex items-center justify-center shadow">
                    <svg className="w-5 h-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="2">
                      <path strokeLinecap="round" strokeLinejoin="round" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" />
                      <path strokeLinecap="round" strokeLinejoin="round" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" />
                    </svg>
                  </div>
                  <h3 className="font-black text-base text-black">Delivery Address</h3>
                </div>
                <div className="bg-gray-50 rounded-xl p-4 border border-gray-200">
                  <div className="font-bold text-base text-black mb-2">{order.customer.name}</div>
              ...
              <div className="bg-white rounded-2xl shadow-xl border border-gray-200 p-5">
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <h3 className="font-black text-base text-black">Order Items ({order.items.length})</h3>
                </div>
                  ...
                  {order.items.map((it) => (
                    <div key={it.id} className="flex items-center gap-3 p-3 bg-gray-50 rounded-lg border border-gray-200">
                      <img
                        src={it.image}
                        alt={it.name}
                        className="w-14 h-14 object-cover rounded-lg bg-white border border-gray-200"
                      />
                      <div className="min-w-0 flex-1">
                        <div
                          className="text-sm font-bold line-clamp-2 text-black"
                          title={it.name}
                          style={{
                            display: "-webkit-box",
                            WebkitLineClamp: 2,
                ...
                <div className="mt-5 pt-4 border-t border-gray-200 space-y-2 text-sm">
                  ...
                  <div className="flex justify-between pt-3 border-t-2 border-black">
                    ...
                    <span className="font-black text-lg">Rs. {total.toLocaleString("en-PK")}</span>
                  </div>
                </div>
              </div>

              {/* Invoice card */}
              <div className="relative bg-gradient-to-br from-black to-gray-900 rounded-2xl shadow-xl border border-gray-800 p-6 text-white overflow-hidden">
                <div className="relative">
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <div>
                      ...
                      <p className="text-gray-400 text-xs">Official Receipt</p>
                    </div>
                  </div>
                  <p className="text-gray-300 text-sm mb-5 leading-relaxed">
                    Download your professional invoice with RS branding
                </p>
                <button
                    onClick={() => downloadInvoice(order)}
                    className="w-full px-5 py-3 bg-white hover:bg-gray-100 text-black rounded-xl font-bold transition-all shadow-lg hover:shadow-xl flex items-center justify-center gap-2 text-sm"
                >
                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="2">
  ...
  return (
    <div className="mt-5">
      ...
      <div className="relative h-2 bg-gray-200 rounded-full overflow-hidden">
        <div
          className="absolute top-0 left-0 h-full bg-gradient-to-r from-black to-gray-700 rounded-full transition-all duration-500"
          style={{ width: `${progress}%` }}
        />
      </div>
      
      {/* Steps */}
      <div className="mt-5 grid grid-cols-6 gap-1 text-[10px] sm:text-xs">
        {STATUS_STEPS.map((s, i) => (
          <div key={s} className="flex flex-col items-center">
            <span
              className={`w-8 h-8 sm:w-9 sm:h-9 grid place-items-center rounded-lg text-xs font-black transition-all mb-2 ${
                i <= current
